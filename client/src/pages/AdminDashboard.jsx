...
import {
    Activity, Users, AlertTriangle, TrendingUp, Clock, Download,
    Filter, MapPin, Calendar, ArrowUpRight, Train, Radio
} from 'lucide-react';
import AdminNavbar from '../components/AdminNavbar';

// Sample Data
const routeOccupancyData = [
    { route: 'Churchgate-Borivali', current: 8500, capacity: 10000, percentage: 85, status: 'critical' },
    { route: 'CST-Thane', current: 7200, capacity: 9000, percentage: 80, status: 'high' },
    { route: 'Andheri-Virar', current: 6800, capacity: 10000, percentage: 68, status: 'moderate' },
    { route: 'Dadar-Kalyan', current: 5500, capacity: 8000, percentage: 69, status: 'moderate' },
    { route: 'Bandra-Churchgate', current: 4200, capacity: 7000, percentage: 60, status: 'normal' },
    { route: 'Kurla-CST', current: 3800, capacity: 6000, percentage: 63, status: 'moderate' },
];
...
const stationHeatmapData = [
    { station: 'Churchgate', '05:00': 30, '06:00': 70, '07:00': 95, '08:00': 98, '09:00': 92, '10:00': 65 },
    { station: 'Marine Lines', '05:00': 25, '06:00': 60, '07:00': 85, '08:00': 90, '09:00': 82, '10:00': 55 },
    { station: 'Charni Road', '05:00': 28, '06:00': 65, '07:00': 88, '08:00': 93, '09:00': 85, '10:00': 58 },
    { station: 'Grant Road', '05:00': 32, '06:00': 72, '07:00': 92, '08:00': 96, '09:00': 88, '10:00': 62 },
    { station: 'Mumbai Central', '05:00': 35, '06:00': 75, '07:00': 94, '08:00': 97, '09:00': 90, '10:00': 68 },
    { station: 'Dadar', '05:00': 40, '06:00': 82, '07:00': 98, '08:00': 99, '09:00': 95, '10:00': 75 },
];
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 via-orange-50/30 to-green-50/30">
            ...
            <div className="max-w-[1600px] mx-auto p-4 md:p-8">
                        ...
                        <div>
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-600 font-medium">Real-time Network Intelligence & Analytics</p>
                                </div>
                            </div>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                    ...
                    <div className="bg-white/90 backdrop-blur-lg rounded-2xl shadow-xl border border-gray-200 p-6">
                        ...
                        <div className="h-80">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={routeOccupancyData} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                                    ...
                                    />
                                    <Legend />
                                    <Bar dataKey="current" fill="#1e40af" name="Current Load" radius={[8, 8, 0, 0]} />
                                    <Bar dataKey="capacity" fill="#94a3b8" name="Capacity" radius={[8, 8, 0, 0]} />
                                </BarChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
                    ...
                    <div className="bg-white/90 backdrop-blur-lg rounded-2xl shadow-xl border border-gray-200 p-6">
                        <h2 className="text-xl font-bold text-brand-navy mb-6">Crowd Density Distribution</h2>
                ...
                <div className="bg-white/90 backdrop-blur-lg rounded-2xl shadow-xl border border-gray-200 p-6 mb-8">
                    ...
                    <div className="h-80">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={hourlyTrafficData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                                <defs>
                                ...
                                />
                                <Legend />
                                <Area type="monotone" dataKey="passengers" stroke="#1e40af" strokeWidth={3} fillOpacity={1} fill="url(#colorPassengers)" name="Passenger Count" />
                                <Area type="monotone" dataKey="capacity" stroke="#dc2626" strokeWidth={2} strokeDasharray="5 5" fillOpacity={1} fill="url(#colorCapacity)" name="Max Capacity" />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                ...
                <div className="bg-white/90 backdrop-blur-lg rounded-2xl shadow-xl border border-gray-200 p-6">
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b-2 border-gray-300">
                                    <th className="px-4 py-3 text-left font-bold text-brand-navy">Station</th>
                                    <th className="px-4 py-3 text-center font-bold text-brand-navy">05:00</th>
                                    <th className="px-4 py-3 text-center font-bold text-brand-navy">06:00</th>
                                    <th className="px-4 py-3 text-center font-bold text-brand-navy">07:00</th>
                                    <th className="px-4 py-3 text-center font-bold text-brand-navy">08:00</th>
                                    <th className="px-4 py-3 text-center font-bold text-brand-navy">09:00</th>
                                    <th className="px-4 py-3 text-center font-bold text-brand-navy">10:00</th>
                                </tr>
                            </thead>
                            <tbody>
                                {stationHeatmapData.map((station, idx) => (
                                    <tr key={idx} className="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                                        <td className="px-4 py-3 font-semibold text-gray-800">{station.station}</td>
                                        {['05:00', '06:00', '07:00', '08:00', '09:00', '10:00'].map((time, i) => (
                    ...
                    <div className="mt-4 flex items-center justify-center gap-6">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-medium text-gray-600">Low (&lt;50%)</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <div className="w-6 h-6 rounded" style={{ backgroundColor: '#eab308' }}></div>
                            <span className="text-sm font-medium text-gray-600">Moderate (50-70%)</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <div className="w-6 h-6 rounded" style={{ backgroundColor: '#f97316' }}></div>
                            <span className="text-sm font-medium text-gray-600">High (70-90%)</span>
                        </div>
                        <div className="flex items-center gap-2">
    ...
    return (
        <div className="bg-white/90 backdrop-blur-md rounded-2xl shadow-lg border border-gray-200 p-6 hover:shadow-2xl hover:scale-105 transition-all duration-300">
            <div className="flex items-center justify-between mb-4">
                ...
                <span className={`text-xs font-bold px-3 py-1.5 rounded-full border ${trendColors[trend]}`}>
                    {change}
                </span>
            </div>
    ...
    return (
        <div className="bg-gradient-to-br from-gray-50 to-white border-2 border-gray-200 rounded-xl p-4 hover:shadow-lg transition-all">
            <div className="flex items-center justify-between mb-3">
                ...
                <span className={`text-xs font-bold px-2 py-1 rounded-full border ${getStatusBg(route.status)}`}>
...
const PeakHourCard = ({ data }) => {
    ...
    return (
        <div className={`rounded-xl p-5 border-2 ${isHighLoad ? 'bg-red-50 border-red-300' : 'bg-green-50 border-green-300'} hover:shadow-lg transition-all`}>
            ...
            <div className="flex items-center justify-between mb-2">
                ...
                <span className={`text-2xl font-black ${isHighLoad ? 'text-red-600' : 'text-green-600'}`}>
                    {data.avgLoad}%
                </span>
            </div>
            ...
            <div className="flex items-center justify-between">
                ...
                <span className={`text-xl font-bold px-2 py-1 rounded ${isHighLoad ? 'bg-red-200 text-red-800' : 'bg-green-200 text-green-800'}`}>
                    {data.incidents}
                </span>
            </div>
        </div>
